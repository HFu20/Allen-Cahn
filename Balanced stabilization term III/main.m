...
% 左侧矩阵
V(2,1) = FunV( xi(2,1) );
A_left = ( 1 + S * V(2,1) * tau ) * I - tau * A;
% 右端项
F = u + tau * S * V(2,1) * u_hat + tau * V(2,1) * dG_hat;
% 解u——线性方程组
v = u;
u = A_left\F;
% 解r——线性方程
r(2,1) = r(1,1) - V(2,1) * TrapezFun( hx, hy, ( dG_hat - S * ( u - u_hat ) ).*( u - v ) ); 
...
%     u = u_hat;
    ...
    A_left = ( 3 + S * V(n+1,1) * 2 * tau ) * I - 2 * tau * A;
    % 右端项
    F = 4 * u - v + S * V(n+1,1) * 2 * tau * u_hat + 2 * tau * V(n+1,1) * dG_hat;
    % 解方程组
    v = u;
    u = A_left\F;
    % 解r——线性方程
    r(n+1,1) = r(n,1) - V(n+1,1) * TrapezFun( hx, hy, ( dG_hat - S * ( u - u_hat ) ).*( u - v ) );
...
%% 数据存储
save E_VV_tau02.mat E;
save uT50_VV_tau02.mat u_2;
save u_max_VV_tau02.mat u_max;

% save xi_VV_01.mat xi;
% save V_VV_01.mat V;
